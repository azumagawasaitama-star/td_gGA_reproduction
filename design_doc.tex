...
\section{ソルバー核：\file{td\_gGA\_solver\_paperconv.py}}
...
ファイル & 役割 \\
\midrule
\file{ga\_mainfin\_routeA.py} & 静的 gGA ソルバー本体（クラス \code{GA}）。
...
\file{td\_gGA\_solver\_routeA.py} & \code{prepare\_full\_params}（dense 演算子ロード、$B\le5$）、
\code{setup\_frequency\_grid}（Gauss--Legendre $\times$ 半円 DOS） \\
\file{td\_gGA\_solver\_routeC.py} & \code{pack\_state/unpack\_state}、
\code{solve\_lambda\_from\_F2}（$\Lambda$ root-finding、フォールバック用）など \\
\bottomrule
\end{longtable}

\textbf{注意}：\file{td\_gGA\_solver\_routeA.py}／\file{routeC.py} 自体の TD ソルバー部は
規約バグ（転置・共役・符号の混在）を含むため使わない。ヘルパー提供のためだけに同梱。
...
\file{migrate\_B135.py} & 旧一枚岩 npz $\to$ \code{data/} レイアウト移行 \\
\bottomrule
\end{longtable}
...
\begin{lstlisting}
[ga_mainfin_routeA + ed_solver]           [paperconv]
...
\item 理論・経緯・展望はそれぞれ
\file{derivation\_F2\_conservation\_proof.md}・\file{STATUS.md}
（+\file{POSTMORTEM\_2026-07-07.md}）・\file{ROADMAP.md} を参照。
\end{itemize}
