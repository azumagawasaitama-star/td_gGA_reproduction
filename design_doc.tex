...
\file{tdvp\_core.py}（旧 \file{td\_gGA\_solver\_routeA.py}） & \code{prepare\_full\_params}（dense 演算子ロード、$B\le5$）、
\code{setup\_frequency\_grid}（Gauss--Legendre $\times$ 半円 DOS） \\
\file{tdvp\_sparse.py}（旧 \file{td\_gGA\_solver\_routeC.py}） & \code{pack\_state/unpack\_state}、
\code{solve\_lambda\_from\_F2}（$\Lambda$ root-finding、フォールバック用）など \\
\bottomrule
\end{longtable}

\textbf{注意}：\file{tdvp\_core.py}／\file{tdvp\_sparse.py} 自体の TD ソルバー部は
